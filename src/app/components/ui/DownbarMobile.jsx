...
export default function DownbarMobile() {
  ...
  return (
    <div className=" fixed bottom-0 w-full overflow-hidden h-[8vh] min-h-[40px] max-h-[60px]  p-[0.8rem] py-4  flex items-center justify-center text-EpaWhite bg-EpaAccent md:hidden">
        ...
        {Object.values(navButtons).map((button) => (
          <li
            className={`h-fit p-2 flex items-center justify-center ${
              currentPath === button.href ? "bg-EpaSecondary" : "bg-transparent"
            }  rounded-lg `}
            key={button.id}
          >
            <Link
